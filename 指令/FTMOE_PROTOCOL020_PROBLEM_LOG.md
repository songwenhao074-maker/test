...
| # | 阶段 | 严重度 | 状态 | 摘要 |
|---|---|---|---|---|
| P15 | S4 | 中 | 预登记（等待全量数据后执行扩展） | 预登记 CPU 网格（1.00–0.70）实测 CPU 主导事件 9–25/6400（0.14–0.39%），即使按 8000 host-step/相位换算也远低于 §13 的 ≥100 水位；正常占比 99.2–99.6% 超出 §11 的 80–95% → 需**数据驱动扩展注册网格**（CPU 向 0.65–0.40 方向、RAM 向 0.25–0.10、Disk 视结果向 0.15–0.12）后再选容量；全部判定只依据 data-only 统计 |
...
#### P15. 预登记网格对 P20 train-cohort 频率不足 —— 扩展网格（数据驱动，预登记）

- **中间数据（cpu 轴 0.70/0.75/0.80，400×16=6400 host-step scored）**：
  | candidate | CPU | RAM | Disk | norm% | dep-rej | mig-rej |
  |---|---|---|---|---|---|---|
  | cpu_0.70 | 25 | 9 | 19 | 99.2% | 9.6% | 21.4% |
  | cpu_0.75 | 9 | 1 | 17 | 99.6% | 9.5% | 23.1% |
  | cpu_0.80 | 16 | 4 | 16 | 99.4% | 5.4% | 13.9% |
- **读数**：CPU 主导事件率仅 0.14–0.39%（019 旧 42-VM 池同尺度约 0.5–0.7%）；按 §13 相位
  水位（500 interval ≈ 8000 host-step 需 target ≥100，即 ≥1.25%）全部不合格；正常占比也
  超 §11 上限（80–95%）。拒绝率均在门内。
- **判定（未动任何模型/标签逻辑）**：预登记 §46 网格只是"推荐初值"（plan §46 原话），
  选择须由 data-only scan 完成 → 需要把各轴向更深的 scale 扩展（cpu {0.65,0.60,0.55,0.50,
  0.45,0.40}；ram {0.25,0.20,0.15,0.10}；disk 视 0.17–0.30 结果决定 {0.15,0.12}）。
  扩展清单在扫描链完成后、跑扩展前正式登记（本条目先占位）；仍只依据数据统计选容量。
