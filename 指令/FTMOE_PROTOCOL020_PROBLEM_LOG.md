...
| # | 阶段 | 严重度 | 状态 | 摘要 |
|---|---|---|---|---|
| P17 | S4(恢复) | 中 | 已登记方向 (b)，执行中 | 用户"继续任务"→ 选定方向 (b)：P20 专用 demand adapter（cpu_upper/ram_mult/disk_mult 可调，默认=016 契约）；探测 v1 候选 10 个（400 interval data-only），目标在拒绝率干净区达到相位投影 ≥100/8000 |
