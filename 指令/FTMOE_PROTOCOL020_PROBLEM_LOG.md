...
| # | 阶段 | 严重度 | 状态 | 摘要 |
|---|---|---|---|---|
| P15 | S4 | 中 | 已登记扩展网格（Extension 1），待全量扫描+补扫后选容量 | 预登记 CPU 网格（1.00–0.70）实测 CPU 主导事件 0–25/6400（cpu_0.9 为 0），相位投影 ≤31/8000 远低于 §13 的 ≥100；正常占比 99.2–99.6% 超 §11 上限 → **数据驱动扩展注册网格**（Extension 1：cpu 增 {0.65,0.60,0.55,0.50,0.45,0.40}、ram 增 {0.25,0.20,0.15,0.10}、disk 增 {0.15,0.12}），仍在 §11/§13 data-only 门禁下选容量 |
