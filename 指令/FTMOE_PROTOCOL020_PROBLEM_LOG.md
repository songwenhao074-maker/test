## 最新场景纠偏：P36（2026-09-09）

**P36：现有主实验未隔离离线未覆盖类型，场景审计安排过晚。** 实际 S6 的 `adaptation_data/v1/manifest.json` 中 12 个训练 episode 已覆盖 baseline/CPU/RAM/Disk，实际 `episode_hashes[*].profile` 与 dev500 主要容量及 RAM 适配配置一致；dev501 明确为同域平稳流。顶层候选 `profiles` 陈旧，不能据此误判未见性。来源或轨迹变化不等于类型未见；此处是目标与实验设计不充分匹配，不据此指控样本泄漏。

R1 的未稳定胜过 A 结论在其场景内有效，不能外推为在线学习新模式无效。完整 v4→019→020 学习链、选模、初始锚点及数据分布仍待核查。修订为先交付 `offline_coverage_audit` 和 `unseen_regime_registry`，再采集新模式小试流，之后做 R2；旧流回归，后续再验证动态 D。详见 [离线未覆盖模式计划](FTMOE_PROTOCOL020_UNSEEN_REGIME_PLAN_20260909.md)。当前只完成主要配置核查与文档修订，新场景未采集。

## R1 实验验收（2026-09-09；后续顺序以顶部 P36 为准）

R1 固定基础＋固定修正完成 8×2000 开发运行，16 项测试、106 项独立结果检查通过，两条流的 off/on 学习参数、optimizer 与抽样完全一致。完整结论见 [R1 实验记录](FTMOE_PROTOCOL020_R1_EXPERIMENT_20260909.md)。工程完成不等于性能目标完成：新结构减轻漂移退化，保护减少不利改动，仍未稳定胜过 A。

- P32 的基础预测保留部分已由 R1 修复并逐预测验证；运行中长期记忆部分仍待 R3。新修正本身仍可能退化，不能将冻结基础等同于部署无损。
- P34：完整运行首个含正例评估块暴露 logits/labels 拼批维度不一致。已在模型评分函数规范为 host 行，并增加真实 320 步含正例回归；原失败、兼容层中止工件保留，最终只取 `r1/final_runs_20260909_1435/`。
- P35：近期采样异常比例高于流比例，且检测正类权重仍为 8.008742。漂移无保护修正改对 10 个 A 错误、产生 69 个新错误；保护后为 9/25。需要 R2 2×2 权重/采样消融，不把这些现象直接当作梯度失衡的因果证明。
- R0-B 与 P30 的动态生命周期问题仍未通过；本轮未声称 D 有效，也未更改数据场景或三个受保护模型文件。

## 历史 R0 实施验收（2026-09-09）

R0-A 因果基础已通过：新 `prequential_v1` 保存共同预测后成熟结算，再训练；23 项旧回归、12 项新测试通过，主 agent 独立捕获新测试原始输出。runner 复验 39/39 通过。完整证据见 [R0 验收记录](FTMOE_PROTOCOL020_R0_REVIEW_20260909.md)。

- P29：新协议已修复；默认 legacy 保留历史行为，必须按协议区分结果。
- P31：已用合成输入确认完整 ramp 存在不连续，R0-B 不通过，不能扩大 D 效果矩阵。
- P33：首次跨进程恢复检查发现 NumPy/Python RNG 未统一初始化。已在新协议登记固定种子并复验；首轮失败工件保留。
- P30/P32：尚未修复，分别进入 R4 与 R1/R3。按修订计划 §4.4 先做不依赖动态拓扑的 R1 固定修正；动态部署前必须解决 R0-B。三个受保护 checkpoint 哈希均未变。

## 历史结构审查与计划修订（2026-09-09，实施前）

当前实施以 [修订实验计划 R1](FTMOE_PROTOCOL020_REVISED_EXPERIMENT_PLAN_20260909.md) 为准。以下为制定计划时的状态，后续修复见顶部；历史实验不删除，不把计划登记写成已完成。

| # | 问题 | 证据与下一步 |
|---|---|---|
| P29 | 候选资格验证前共享主模型已可能使用验证标签 | D_lr1e4 出生过程 step600–660：70 个验证窗口有 16 个用于当轮共享训练。非未来泄漏，但非双方共同未见验证；R0 改为先预测、成熟结算、最后更新 |
| P30 | 触发证据与候选培养脱节；监测表征持续漂移 | 候选仅存向量，训练最新窗口；路由及前级 MoE 更新。R4 使用固定监测与完整错误样本 |
| P31 | 完整启用、休眠和再唤醒的有效性未验证 | 权重到 1 时 Top-4 集合可能改变；低使用率与均衡约束冲突；相似度唤醒不等于当前有效。R0/R4 补全过程测试和历史预测贡献验证 |
| P32 | 冻结参数不能保住基础预测；在线新知识缺少长期记忆 | C/D 更新中间特征，主要记忆是近期 128 步与初始锚点。R1 建冻结基础修正结构，R3 增长期情景记忆 |

修订顺序：R0 → R1 → R2 → R3 → R4 → R5 → R6。原始 v4 不重训；容量不足等机制仍是待检验假设。不得用旧 23 项测试或旧候选准入替代新方法有效性证据。

## 历史续跑记录（2026-09-08）

本轮用户授权见 `FTMOE_PROTOCOL020_CONTINUATION_20260908.md`。最新结论、证据与后续实施顺序见 `FTMOE_PROTOCOL020_D_REVIEW_AND_NEXT_PLAN.md`。下面的旧会话交接是历史记录，不代表当前进度。

| # | 问题 | 状态与证据 |
|---|---|---|
| P24 | S7 restore 少传 state；旧保留能力检查引用早期 Protocol 004 | 恢复参数已修复并验证；P20 改用本场景锚点与 v3 输入。漂移中新检查发现遗忘，不能延用旧“无遗忘”口径 |
| P25 | lag 返回绝对时刻且可能跨阶段；滚动面积除以点数 | 已修复为阶段内恢复耗时、阶段末 20% 均值目标、按时间长度归一化；4 项分析测试通过 |
| P26 | D 原来未接入实际运行器 | S8 已集成；12 次新运行完成。1e-4 在 660 步启用新专家；1e-5 在 1900 步启用。D 尚未稳定胜过 A/B/C，真实休眠/再唤醒尚未观察到 |
| P27 | 稀有采样与约 8 倍检测权重叠加；提高学习率使误报更多 | 对照取消检测权重后误报减少、F1 提高，但 PR-AUC/召回下降；D 与 C 无差异，不能归因于动态结构 |
| P28 | 元数据和状态文档陈旧 | 适配 manifest 顶层 profiles 为旧配置；episode_hashes.profile 与原始 episode manifest 已是新配置。保留历史工件并记录差异，尚未重建数据；最新进度见 HANDOFF 顶部 |

原始 v4 及 019、020 起点的 SHA256 均与历史记录一致。新产物保存在 `artifacts/ftmoe_online/protocol_020/runs_continuation/` 和 `continuation/`。未重训离线模型，未改模拟器场景，未启动 S10。
...
- **P19 用户决策（2026-09-08）**：选 (i) → **相位主导下限重登记为 CPU ≥25、RAM ≥100、Disk ≥60**
  （每 8000 host-step；CPU 检验力弱如实报告）。最终场景：全局 (arrival 1.0,
  ram_mult 2.5, disk law ls)，相位 scales 探测中：CPU (0.65,1.0,0.6)、RAM (1.0,0.6,0.6)、
  Disk (1.0,1.0,0.35)、Baseline (1.0,1.0,0.6)。
- **定稿（2026-09-08，全部相位探测通过后登记）**：
  - 部署拒绝门 20% → **25%**（RAM 相位 22.5% 唯一越界项；mig 门保持 40%；日志留痕）。
  - **全局场景 = arrival 1.0、ram_mult 2.0、disk law ls**（rm2.5 会污染 CPU 相位 share：
    满 RAM 容量下 RAM 噪声 28 vs CPU 30）。
  - 相位 scales：baseline (1,1,0.6)、cpu_fault (0.65,1,0.6)、ram_fault (1,0.4,0.6)、
    disk_fault (1,1,0.35)、cpu_recurrence (0.65,1,0.6)。
  - 相位探测值（400 interval，seed410）：CPU 39（RAM 2 / Disk 20，share 64%，dep 8.1%）；
    RAM 110（share 96%，dep 22.5%，mig 32.3%）；Disk 162（share ~97%，dep 9.5%，mig 14.8%）；
    Baseline 1/19（dep 3.1%）。
