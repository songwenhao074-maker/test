...
Both candidates use the same raw_next_fault target and the same independently
observed historical anchor whose labels are reconstructed from raw[i+1].  Old
P20/P23 stored tolerance labels are forbidden.
"""
...
def _run_one(bundle, anchor, stream_dir, out_dir, update_every):
    session = Protocol024Session(
        ...
        run_id="budget_u%d_rawnext_anchor_v2" % update_every,
...
def run(stream_dir, out_path):
    ...
    if meta.get("legacy_stored_labels_used") is not False:
        raise AssertionError("raw-next budget anchor must reject legacy stored labels")
    if meta.get("new_response_law_stream_used") is not False:
        raise AssertionError("budget anchor must be independent of the new stream")

    root = out_path.parent / "runs"
    u4 = _run_one(bundle, anchor, stream_dir, root / "C_u4", 4)
    bundle_u1 = s4.build_replay(stream_dir)
    u1 = _run_one(bundle_u1, anchor, stream_dir, root / "C_u1", 1)
