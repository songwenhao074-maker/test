"""Collect the single registered protocol016 adapted BWGD2 pilot."""
import argparse
import contextlib
import hashlib
import json
...
def collect(seed, steps, output):
    if seed != 301:raise ValueError('Unregistered replay seed')
    if steps != 300:raise ValueError('Unregistered horizon')
    if output.exists():raise FileExistsError(output)
    output.mkdir(parents=True)
    console=sys.stdout
    try:
        configure();guard();os.chdir(ROOT)
        import numpy as np
        import torch
        import psutil
        ...
        with (output/'generation.log').open('w',encoding='utf8') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
            ...
            dc=RPiEdge(16);workload=AdaptedBWGD2(1,1.5,seed);scheduler=GOBIScheduler('energy_latency_16')
            ...
            sources=[ROOT/'prepare_ftmoe_adapted_bwgd2.py', ROOT/'artifacts/ftmoe_online/adapted_bwgd2_016/protocol.json', ROOT/'artifacts/ftmoe_online/adapted_bwgd2_016/disk_law.json', scheduler_weight]
            ...
            manifest={'schema_version':1,'seed':seed,'steps':steps,'guard_steps':1,'workload':'adapted_BWGD2_protocol016',
                'interval_seconds':300,'hosts':16,'containers':16,'arrival_mean':1,'arrival_sigma':1.5,
                'capacity_scales':{'CPU_CAP_SCALE':.8,'DISK_CAP_SCALE':.25},'artificial_drift':False,
                'synthetic_dynamic_disk':True,'demand_adapter':{'cpu_positive_clip':[2,1860],'ram_multiplier':2,'io_constant':1},
        ...
        failure={'seed':seed,'steps':steps,'error':type(exc).__name__+': '+str(exc),'traceback':traceback.format_exc(),
                 ...
                 'simulator_behavior_modified':True,'authorized_configuration':'protocol016','next_action':'Report to user before simulator changes'}
...
if __name__=='__main__':
    ...
    collect(args.seed,args.steps,args.output or ROOT/f'artifacts/ftmoe_online/adapted_bwgd2_016/streams/seed{args.seed}_steps{args.steps}')
