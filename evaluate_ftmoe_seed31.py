"""One-shot evaluation of the frozen ablation chain on replay seed 31."""
...
DATA = Path("recovery/PreGANSrc/data/ftmoe_replay_holdout_s31")
ARTIFACT_ROOT = Path("artifacts/ftmoe_ablation/replay_validation")
OUTPUT = Path("artifacts/ftmoe_ablation/formal_results_seed31")
SEEDS = (1, 2, 6)
RUNS = {
    "v0": ARTIFACT_ROOT / "s31_001_v0",
    "v1": ARTIFACT_ROOT / "s31_002_v1",
    "v2": ARTIFACT_ROOT / "s31_003_v2",
    "v3": ARTIFACT_ROOT / "s31_006_v3_budget3",
    "v4": ARTIFACT_ROOT / "s31_009_v4_finetune",
}
SELECTION = {
    "v0": "F1",
    "v1": "F1 + 0.2*HR@100 + 0.1*NDCG@100",
    "v2": "one epoch; F1 + 0.2*HR@100 + 0.1*NDCG@100",
    "v3": "three epochs; F1 + HR@100 + NDCG@100",
    "v4": (
        "30 CMHA-only epochs then 15 full-model epochs; "
        "F1 + HR@100 + NDCG@100"
    ),
}
...
def test_loader() -> DataLoader:
    ...
    dataset = ExistingQoSDataset(
        ...
        np.asarray(labels), list(range(1010, 1212)),
    )
...
def main() -> None:
    ...
    lock = {
        "status": "frozen-before-first-seed31-evaluation",
        "dataset": str(DATA),
        "training_rows": [0, 808],
        "development_validation_rows": [808, 1010],
        "locked_test_rows": [1010, 1212],
        "locked_test_replay_seed": 31,
