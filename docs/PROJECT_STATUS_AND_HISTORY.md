...
本文件以下内容为按日期保存的历史记录，旧“当前/唯一/下一步”只描述当时状态。
当前执行请读 [交接说明](GITHUB_EXPERIMENT_HANDOFF.md) 与 [027结果](PROTOCOL027_RESULTS.md)。027的实现及失败证据已经整合进main；旧D_eligible与026五方法计划不再支配当前任务。

---

# 2026-09-21 当前状态补充

用户将当前重点限定为动态残差D对固定残差C，并要求此后每份指示只规划一个任务。
已发布 [Protocol-027单任务指示](PROTOCOL027_SINGLE_TASK_DIRECTIVE_20260921.md)：复用025六业务更替数据，C_fixed5/D_dynamic各一次开发运行，交付后停止。A/B、其他C、消融与确认不在本次范围。

027目前仅设计发布，尚未实现新入口或运行模型。026五方法计划被替代，025原审计失败与所有旧结果保留。原有正常保护集修订和数据恢复办法继续作为技术依赖。

最新完整性能结果仍为024 v2c；下文均为历史记录，旧“当前唯一入口”不代表当前执行授权。

---

# 2026-09-18 当前状态补充

v2c复现AP均值D−C=-0.00009536，2/6正，仍无优势。Protocol-025数据已生成；
强化审计唯一失败项为F0 guard 960正常、0故障，尚无Protocol-025模型比较。

本次保留Protocol-025负门禁结果并注册Protocol-026：复用同一物理数据，将F0用于正常回归保护。
另已归档即将到期的恢复产物（run35292728416），没有运行模拟或模型。
当前唯一入口：[指示](PROTOCOL026_NEXT_DIRECTIVE_20260918.md)、[审阅](PROTOCOL025_REVIEW_20260918.md)。
下文保留历史判断。确认701–703、测试201–205继续封存。
