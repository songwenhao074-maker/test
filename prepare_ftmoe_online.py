"""Collect model-independent native BWGD2 trajectories without changing simulator behavior."""
import argparse
import contextlib
import hashlib
import json
...
def collect(seed, steps, output):
    if seed not in (301,302,401,402,403):raise ValueError('Unregistered replay seed')
    if steps not in (300,1000,2000):raise ValueError('Unregistered horizon')
    ...
    try:
        ...
        with (output/'generation.log').open('w',encoding='utf8') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
            ...
            dc=RPiEdge(16);workload=BWGD2(1,1.5);scheduler=GOBIScheduler('energy_latency_16')
            ...
            sources=[ROOT/'prepare_ftmoe_online.py',scheduler_weight]
            ...
            manifest={'schema_version':1,'seed':seed,'steps':steps,'guard_steps':1,'workload':'native_BWGD2',
                'interval_seconds':300,'hosts':16,'containers':16,'arrival_mean':1,'arrival_sigma':1.5,
                'capacity_scales':{'CPU_CAP_SCALE':1.,'DISK_CAP_SCALE':1.},'artificial_drift':False,
        ...
        failure={'seed':seed,'steps':steps,'error':type(exc).__name__+': '+str(exc),'traceback':traceback.format_exc(),
                 ...
                 'simulator_behavior_modified':False,'next_action':'Report to user before simulator changes'}
...
if __name__=='__main__':
    ...
    collect(args.seed,args.steps,args.output or ROOT/f'artifacts/ftmoe_online/streams/seed{args.seed}_steps{args.steps}')
