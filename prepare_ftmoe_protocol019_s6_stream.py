"""Protocol 019 S6 — stationary long-stream collector (2000 intervals, seed 304).

Same adapted-BWGD2 (protocol-016 contract) generator as the S2 dev stream,
newly registered for the stationary validation phase.  Source isolation:
seed 304 is disjoint from S2 dev stream (303) and S3 episodes (401-408).
"""
import argparse
import contextlib
import hashlib
...
OUT = ROOT / "artifacts/ftmoe_online/protocol_019/stationary_streams"
ALLOWED_SEEDS = {304}
ALLOWED_STEPS = {2000}
...
def register(seed, steps):
    path = OUT / "s6_stream_config.json"
    if not path.exists():
        config = {
            "schema_version": 1, "protocol": "019", "phase": "S6",
            "name": "stationary long stream", "replay_seed": seed, "steps": steps,
            "guard_steps": 1, "workload": "adapted_BWGD2_protocol019",
            ...
            "demand_adapter": {"cpu_positive_clip": [2, 1860], "ram_multiplier": 2,
                               "io_constant": 1},
            "execution_boundary": ("S6 stationary sanity only: A must stay stable, "
                                   "B/C must not forget, D must not grow experts and "
                                   "must not be materially worse than C."),
        }
...
def collect(seed, steps, output):
    if seed not in ALLOWED_SEEDS:
        raise ValueError(f"Unregistered S6 seed: {seed}")
    if steps not in ALLOWED_STEPS:
        raise ValueError(f"Unregistered S6 horizon: {steps}")
    if output.exists():
    ...
    try:
        ...
        with (output / "generation.log").open("w", encoding="utf8") as log, \
            ...
            sources = [ROOT / "prepare_ftmoe_protocol019_s6_stream.py",
            ...
            manifest = {"schema_version": 1, "protocol": "019", "phase": "S6",
                        ...
                        "s6_config_sha256": sha(OUT / "s6_stream_config.json"),
