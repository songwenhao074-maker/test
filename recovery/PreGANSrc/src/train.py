...
def backprop(epoch, model, train_time_data, train_schedule_data, anomaly_data, class_data, optimizer, training = True, extra_loss_fn = None):
	global PROTO_UPDATE_FACTOR, num_ones, num_zero
	num_zero, num_ones = 1, 1
	aloss_list, tloss_list, sloss_list = [], [], []
	for i in tqdm(range(train_time_data.shape[0]), leave=False, position=1):
		...
		sloss = model.routing_regularization() if hasattr(model, 'routing_regularization') else torch.tensor(0.0, dtype=torch.double)
		aloss_list.append(aloss.item()); tloss_list.append(tloss.item()); sloss_list.append(sloss.item())
		if hasattr(model, 'loss_weights'):
			alpha1, alpha2, alpha3 = model.loss_weights
			loss = alpha1 * aloss + alpha2 * tloss + alpha3 * sloss
		else:
			loss = aloss + tloss
		if extra_loss_fn is not None:
			loss = loss + extra_loss_fn(model, i, train_time_data,
										train_schedule_data, output)
		if training:
			optimizer.zero_grad()
			loss.backward()
			optimizer.step()
	mean_loss = np.mean(aloss_list) + np.mean(tloss_list) + np.mean(sloss_list)
	tqdm.write(f'Epoch {epoch},\tLoss = {mean_loss},\tALoss = {np.mean(aloss_list)},\tTLoss = {np.mean(tloss_list)},\tSLoss = {np.mean(sloss_list)}')
	factor = PROTO_UPDATE_FACTOR + PROTO_UPDATE_MIN
	return mean_loss, factor
...
def accuracy(model, train_time_data, train_schedule_data, anomaly_data, class_data, model_plotter):
	...
	return anomaly_correct / len(train_time_data), (class_correct / class_total if class_total > 0 else 0.0)
