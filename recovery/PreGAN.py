...
class PreGANRecovery(Recovery):
    # Per-method GAN identity: PreGAN keeps its own GAN checkpoint
    # (separate directory + method prefix), never shared with PreGAN+ / FT-MoE.
    gan_key = 'PreGAN'
    ...
    def __init__(self, hosts, env, training = False):
        ...
        # the per-method checkpoint FILE prefix is handled by gan_ckpt_name().
    ...
    def gan_ckpt_name(self, model_name):
        """Per-method, per-seed GAN checkpoint file name (same convention as
        PreGANPlusRecovery)."""
    ...
    def load_models(self):
        ...
        self.gen, self.disc, self.gopt, self.dopt, self.epoch, self.accuracy_list = \
            load_gan(model_folder, self.gan_ckpt_name(self.gen_name), self.gan_ckpt_name(self.disc_name), self.gen_name, self.disc_name)
    ...
    def train_gan(self, embedding, schedule_data):
        ...
        save_gan(model_folder, self.gan_ckpt_name(self.gen_name), self.gan_ckpt_name(self.disc_name), \
                self.gen, self.disc, self.gopt, self.dopt, self.epoch, self.accuracy_list)
    ...
    def recover_decision(self, embedding, schedule_data, original_decision):
        ...
        for cid in np.concatenate(host_alloc):
            cid = int(cid)
            # NOTE: target host from the GAN-generated schedule, not the
            # GOBI schedule passed in (see PreGANPlus.recover_decision).
