...
from recovery.FTMoE import FTMoERecovery
from recovery.FTMoEProgressive import FTMoEProgressiveRecovery
...
parser.add_option("-r", "--recovery", action="store", dest="recovery", default="ftmoe",
					help="Fault recovery: ftmoe, preganplus, pregan, pcft, dftm, eclb, cmodlb")
...
def initalizeEnvironment(environment, logger):
	...
	# Initialize recovery
	recovery_types = {
		'ftmoe': FTMoEProgressiveRecovery,
	...
	if opts.recovery.lower() not in recovery_types:
		raise ValueError('Unknown recovery method: ' + opts.recovery)
	recovery = recovery_types[opts.recovery.lower()](HOSTS, environment, training = True)
...
if __name__ == '__main__':
	...
	train_data = getattr(recovery, 'train_time_data', None)
	env_name = recovery.env_name
	if train_data is not None:
		stats.report_metrics(recovery.model, train_data, env_name)
